...
# -------------------------------------------------------------
# USER-SCOPED BOT FACTORY (Multi-Tenant Engine)
# -------------------------------------------------------------
def create_user_bot_app(
    user_id: str,
    token: str,
    allowed_telegram_ids: Optional[List[int]] = None
) -> Optional[Application]:
    """
    Factory function to create a fully isolated Telegram Bot Application for a specific user.
    """
    token_clean = token.strip()
    if not token_clean:
        return None

    allowed_ids = allowed_telegram_ids or []

    def check_user_allowed(tg_id: int) -> bool:
        if not allowed_ids:
            return True
        return tg_id in allowed_ids

    def get_em() -> ExcelManager:
        return ExcelManager(user_id=user_id)

    app = Application.builder().token(token_clean).build()

    async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
        u = update.effective_user
        if not check_user_allowed(u.id):
            await update.message.reply_text(
                f"⛔ *Akses Ditolak*\n\nUser ID Telegram Anda (`{u.id}`) belum diizinkan di dashboard pemilik bot ini.",
                parse_mode="Markdown"
            )
            return
        ...
        welcome_text = (
            f"👋 Halo, *{u.first_name}*!\n\n"
            f"Selamat datang di Bot Money Tracker pribadi Anda.\n"
            f"Akun Dashboard Terhubung: *{user_id}*\n\n"
            ...
            f"• `/saldo` : Cek saldo rekening & dompet\n"
            f"• `/aset` : Portofolio saham, crypto & emas\n"
            f"• `/cicilan` : Daftar cicilan & jatuh tempo\n"
    ...
    async def cmd_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(
            f"🆔 *User ID Telegram Anda:* `{update.effective_user.id}`\n\n"
            f"Masukkan nomor ID ini pada menu *Pengaturan & Bot* di Web Dashboard.",
            parse_mode="Markdown"
        )
    ...
    async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not check_user_allowed(u_id):
            await update.message.reply_text(
                f"⛔ User ID Anda (`{u_id}`) belum diizinkan di dashboard.",
                parse_mode="Markdown"
            )
            return
        ...
        if add_inst:
            try:
                ...
                msg = (
                    ...
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"💾 _Tersimpan di Akun ({user_id}) & Dashboard Web!_"
                )
                await update.message.reply_text(msg, parse_mode="Markdown")
                return
        ...
        if data == "btn_rekap":
            ...
            try:
                ...
                msg = (
                    ...
                    f"📈 *Progress:* Bulan ke-*{inst['cicilan_ke']}* dari *{inst['tenor']}*\n"
                    f"⏳ *Sisa Tenor:* {inst['sisa_tenor']} Bulan\n"
...
def create_bot_app() -> Optional[Application]:
    """Fallback factory for primary user bot configured via environment variables."""
    token = config.TELEGRAM_BOT_TOKEN
    if not token:
        return None
    primary_user = config.TELEGRAM_PRIMARY_USER_ID or "mghazalinurrahman939@gmail.com"
    return create_user_bot_app(
        user_id=primary_user,
        token=token,
        allowed_telegram_ids=config.ALLOWED_TELEGRAM_USERS
    )
